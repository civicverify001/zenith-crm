...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!phoneNumbersRes.ok) {
      ...
      console.error('[send-sms] Failed to fetch phone numbers:', phoneNumbersRes.status, errText)
      return res.status(500).json({ error: 'Failed to connect to OpenPhone' })
    }

    const phoneNumbers = await phoneNumbersRes.json()
    const allNumbers = phoneNumbers.data || []
    const opDigits = OPENPHONE_NUMBER.replace(/\D/g, '')

    // Flexible matching — compare digits only
    let ourNumber = null
    for (const pn of allNumbers) {
      const pnNum = pn.formattedNumber || pn.number || pn.phoneNumber || ''
      const pnDigits = pnNum.replace(/\D/g, '')
      if (pnDigits === opDigits || pnDigits.endsWith(opDigits) || opDigits.endsWith(pnDigits)) {
        ourNumber = pn
        break
      }
    }

    // If still not found, just use the first number in the account
    if (!ourNumber && allNumbers.length > 0) {
      ourNumber = allNumbers[0]
      console.log('[send-sms] Using first available number:', ourNumber.id)
    }

    if (!ourNumber) {
      console.error('[send-sms] No phone numbers in OpenPhone account')
      return res.status(500).json({ error: 'No phone numbers found in OpenPhone account' })
    }

    console.log('[send-sms] Using phone number:', ourNumber.id, ourNumber.formattedNumber || ourNumber.number)
    ...
    if (!sendRes.ok) {
      console.error('[send-sms] OpenPhone send error:', sendRes.status, JSON.stringify(sendData))
      return res.status(sendRes.status).json({
        error: 'Failed to send SMS',
        detail: sendData.message || sendData.error || JSON.stringify(sendData),
      })
    }

    const messageId = sendData.data ? sendData.data.id : sendData.id || null
    ...
    await supabase.from('communications_log').insert({
      ...
      metadata: { source: 'crm_send_button' },
    ...
    if (entity_type && entity_id) {
      ...
      await supabase.from(activityTable).insert({
        ...
        metadata: { channel: 'sms', direction: 'outbound', to: toFormatted, body: body, external_id: messageId },
    ...
    console.log('[send-sms] Sent successfully:', { to: toFormatted, messageId })
