...
// ─── PDF header using real logo ───────────────────────────────────
function getPdfHeader(logo: string) {
  return `<div style="background:#0a2540;padding:24px 32px;text-align:center;border-radius:8px 8px 0 0;">
    <img src="${logo}" style="height:64px;max-width:320px;object-fit:contain;" alt="Zenith Pure Solutions" />
...
function generateQuoteHTML(quote: any, customer: any, lineItems: any[]): string {
  ...
  const rowsHTML = productItems.map((li: any) => `
    <tr>
      <td style="padding:10px 16px;border-bottom:1px solid #f1f5f9;font-size:13px;color:#334155;"><strong>${li.description}</strong></td>
  ...
  const plansHTML = planItems.length > 0 ? `
    <div style="background:#fffbeb;border:1px solid #fcd34d;border-radius:8px;padding:16px;margin-top:16px;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0">
        ${planItems.map((p: any) => `<tr><td style="font-size:13px;color:#78350f;padding:4px 0;">${p.description}</td><td style="font-size:13px;color:#92400e;font-weight:600;text-align:right;padding:4px 0;">${fmt(parseFloat(p.unit_price)||0)}</td></tr>`).join('')}
      </table>
    </div>` : ''
...
<style>* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: Georgia, serif; color: #1a1a2e; background: white; padding: 40px; max-width: 800px; margin: 0 auto; } @media print { .no-print { display: none !important; } body { padding: 20px; } }</style>
</head><body>
${PDF_PRINT_BTN}
${getPdfHeader(ZENITH_LOGO_FULL)}
<div style="background:#0a2540;color:white;padding:16px 32px;border-top:1px solid rgba(255,255,255,0.1);">
  <table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td><div style="font-size:20px;font-weight:900;letter-spacing:1px;">QUOTATION # ${quote.quote_number}</div></td>
    <td style="text-align:right;vertical-align:top;"><div style="font-size:11px;color:#93c5fd;">6951 E 30th, Suite B · Indianapolis IN 46219</div></td>
  </tr></table>
</div>
...
<div style="border:1px solid #e2e8f0;border-top:none;padding:14px 28px;background:#f8fafc;">
  <table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td><div style="font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;">Quotation Date</div><div style="font-size:13px;color:#1e293b;margin-top:2px;">${quote.created_at?new Date(quote.created_at).toLocaleDateString():today()}</div></td>
    <td><div style="font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;">Expiration</div><div style="font-size:13px;color:#dc2626;font-weight:600;margin-top:2px;">${quote.valid_until?new Date(quote.valid_until).toLocaleDateString():'—'}</div></td>
    <td><div style="font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;">Sales Consultant</div><div style="font-size:13px;color:#1e293b;margin-top:2px;">Kuldeep</div></td>
    <td><div style="font-size:10px;font-weight:700;color:#64748b;text-transform:uppercase;">Type</div><div style="font-size:13px;color:#1e293b;margin-top:2px;">${isRental?'Rental':'Purchase'}</div></td>
  </tr></table>
</div>
...
function generateAgreementHTML(agreement: any, customer: any, terms: any[]): string {
  const termBlocksHTML = terms.map((block: any) => `
    <div class="section">
      ...
      <p>${(block.content||'').replace(/\[INSTALL_FEE\]/g,fmt(agreement.install_fee||0)).replace(/\[MONTHLY_AMOUNT\]/g,fmt(agreement.monthly_amount||0)).replace(/\n/g,'<br/>')}</p>
    </div>
  `).join('')
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .notice { background: #fffbeb; border: 1px solid #fcd34d; border-top: none; padding: 12px 20px; text-align: center; font-size: 11px; font-weight: bold; color: #92400e; text-transform: uppercase; }
...
<style>* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: Georgia, serif; color: #1a1a2e; background: white; padding: 40px; max-width: 800px; margin: 0 auto; } @media print { body { padding: 20px; } .no-print { display: none !important; } }</style>
  ...
  <div style="background:#0a2540;color:white;padding:16px 32px;border-top:1px solid rgba(255,255,255,0.1);">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td><div style="font-size:24px;font-weight:900;letter-spacing:1px;">INVOICE</div><div style="color:#93c5fd;font-size:13px;margin-top:2px;">${invoice.invoice_number}</div></td>
      <td style="text-align:right;"><div style="font-size:32px;font-weight:900;">${fmt(total)}</div><div style="color:#93c5fd;font-size:11px;margin-top:2px;">Total Amount Due</div></td>
    </tr></table>
  </div>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:32px;padding:20px 24px;border:1px solid #e2e8f0;border-top:none;">
    ...
    <div style="text-align:right;">
      ...
      <div style="font-size:12px;color:#64748b;">6951 E 30th St, Suite B · Indianapolis, IN 46219</div>
  ...
  <table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0;border-top:none;">
    <thead><tr style="background:#f8fafc;border-bottom:1px solid #e2e8f0;">
      <th style="padding:10px 24px;text-align:left;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;">Description</th>
      <th style="padding:10px 16px;text-align:center;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;">Qty</th>
      <th style="padding:10px 16px;text-align:right;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;">Unit Price</th>
      <th style="padding:10px 24px;text-align:right;font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;">Amount</th>
    </tr></thead>
    <tbody>${rowsHTML}</tbody>
  </table>
...
function generateDrillingConsentHTML(form: any, signatureUrl: string | null): string {
  ...
  ${PDF_PRINT_BTN}${getPdfHeader(ZENITH_LOGO_FULL)}
...
function generateHandoverHTML(form: any, signatureUrl: string | null, customer: any): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"/><title>RO Handover — ${customerName}</title>
  <style>${PDF_BASE_STYLES}.tds-box{background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:20px;text-align:center;margin:16px 0;}.tds-label{font-size:11px;font-weight:bold;color:#3b82f6;text-transform:uppercase;letter-spacing:1px;}.tds-value{font-size:36px;font-weight:bold;margin-top:4px;}.tds-unit{font-size:14px;color:#64748b;margin-top:2px;}</style>
  </head><body>
  ${PDF_PRINT_BTN}${getPdfHeader(ZENITH_LOGO_FULL)}
...
export function CustomerDocumentsTab({ customerId }: Props) {
  ...
  async function handleDownloadQuote(q: any) {
    ...
    try {
      let { data: lineItems } = await supabase.from('document_line_items').select('*').eq('document_id', q.id).order('sort_order')
      if (!lineItems || lineItems.length === 0) {
        const { data: legacy } = await supabase.from('quote_line_items').select('*').eq('quote_id', q.id).order('sort_order')
        lineItems = legacy || []
      }
  ...
  async function handleDownloadInvoice(inv: any) {
    ...
    try {
      ...
      if (inv.line_items_snapshot?.length) {
        ...
        const { data: quoteData } = await supabase.from('quotes').select('id').eq('customer_id', customerId).in('status', ['accepted', 'signed']).order('created_at', { ascending: false }).limit(5)
        if (quoteData?.length) {
          for (const qt of quoteData) {
            const { data: items } = await supabase.from('document_line_items').select('*').eq('document_id', qt.id).order('sort_order')
  ...
  function handleDownloadForm(form: any) {
    ...
    try {
      ...
      if (form.form_type === 'ro_drilling_consent') openPrintWindow(generateDrillingConsentHTML(form, signatureUrl), `RO-Drilling-Consent-${form.id}.html`)
      else if (form.form_type === 'ro_handover') openPrintWindow(generateHandoverHTML(form, signatureUrl, refs), `RO-Handover-${form.id}.html`)
    ...
    <div className="space-y-5">
            ...
            <DocCard key={agr.id}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {agr.signed_at && <DownloadBtn id={agr.id} label="⬇ Full PDF" onClick={() => handleDownloadAgreement(agr)} />}
                </div>
              </div>
          ...
          {(acceptedQuotes as any[]).map((q: any) => (
            <DocCard key={q.id}>
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm">📋</span>
                  <span className="text-sm font-medium text-white">{q.commercial_type === 'rental' ? 'Rental Quote' : q.commercial_type === 'financed' ? 'Financed Quote' : 'Purchase Quote'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ backgroundColor: 'rgba(56,189,248,0.15)', color: '#38bdf8' }}>{q.status === 'signed' ? 'Signed' : 'Accepted'}</span>
          ...
          {(paidInvoices as any[]).map((inv: any) => (
            <DocCard key={inv.id}>
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ backgroundColor: 'rgba(74,222,128,0.15)', color: '#4ade80' }}>{inv.status === 'partial' ? 'Partial' : 'Paid'}</span>
            ...
            return (
              <DocCard key={form.id}>
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm">🤝</span>
                    <span className="text-sm font-medium text-white">{FORM_LABELS[form.form_type] || form.form_type?.replace(/_/g, ' ') || 'Handover Form'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-xs px-2 py-0.5 rounded-full font-semibold bg-green-500/20 text-green-400">Completed</span>
                    {canDownload && <DownloadBtn id={form.id} label="⬇ Full PDF" onClick={() => handleDownloadForm(form)} />}
                  </div>
                </div>
                <div className="text-xs text-muted space-y-0.5">
                  <div>Submitted: {formatDateTime(form.submitted_at)}</div>
                  {rd.customer_name && <div>Signed by: {rd.customer_name}</div>}
                  {rd.tds_reading && <div className="flex items-center gap-1 mt-1"><span className="text-blue-400 font-semibold">TDS: {rd.tds_reading} ppm</span><span className="text-gray-500">post-filter reading</span></div>}
                </div>
        ...
        <DocSection title="Consent Signatures" icon="✍️" count={signatures.length}>
          {signatures.map((sig: any) => (
            <DocCard key={sig.id}>
              ...
              {sig.signature_url && <div className="mt-2"><img src={sig.signature_url} alt="Signature" className="h-12 bg-white rounded px-2 py-1 cursor-pointer" onClick={() => setLightboxUrl(sig.signature_url)} /></div>}
            </DocCard>
          ))}
        </DocSection>
      )}
      ...
      {pendingProofs.length > 0 && (
        <DocSection title="Pending Review" icon="📋" count={pendingProofs.length} urgentColor>
          {pendingProofs.map(proof => <ProofCard key={proof.id} proof={proof} canReview={role === 'admin'} onReview={() => setReviewModal(proof)} />)}
        </DocSection>
      )}

      {reviewedProofs.length > 0 && (
        <DocSection title="Compliance Documents" icon="📄" count={reviewedProofs.length}>
          {reviewedProofs.map(proof => <ProofCard key={proof.id} proof={proof} canReview={false} />)}
        </DocSection>
      )}
      ...
      {reviewModal && <ProofReviewModal proof={reviewModal} customerId={customerId} onClose={() => setReviewModal(null)} onCompleted={() => setReviewModal(null)} />}
...
function DocSection({ title, icon, count, urgentColor, children }: { title: string; icon: string; count: number; urgentColor?: boolean; children: React.ReactNode }) {
  return (
  ...
  return (
    <DocCard>
      <div className="flex items-center justify-between mb-1">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ backgroundColor: `${reviewStyle.color}20`, color: reviewStyle.color }}>{reviewStyle.label}</span>
          {canReview && proof.review_status === 'pending' && onReview && (
            <button onClick={onReview} className="text-xs px-2 py-0.5 rounded-lg font-semibold" style={{ backgroundColor: 'rgba(56,189,248,0.12)', color: '#38bdf8', border: '1px solid rgba(56,189,248,0.25)' }}>Review</button>
          )}
        </div>
      </div>
      <div className="text-xs text-muted space-y-0.5">
      ...
      {proof.proof_url && <a href={proof.proof_url} target="_blank" rel="noopener noreferrer" className="text-xs mt-2 inline-block" style={{ color: '#38bdf8' }}>View document ↗</a>}
    </DocCard>
  )
}
