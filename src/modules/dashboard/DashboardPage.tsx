...
function useFailedPayments() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const ids = [...new Set(data.map((t: any) => t.customer_id).filter(Boolean))]
      const { data: custs } = await supabase.from('customers').select('id, full_name, phone').in('id', ids)
      const cm = Object.fromEntries((custs || []).map((c: any) => [c.id, c]))
      return data.map((t: any) => ({ ...t, customers: cm[t.customer_id] || null }))
    },
    refetchInterval: 30_000,
  })
}
          ...
          return (
            <div key={stage.key} style={{ marginBottom: i < stages.length - 1 ? 10 : 0 }}>
              ...
              <div style={{ height: 8, background: '#0d1a26', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', borderRadius: 4, width: `${pct}%`, background: stage.count > 0 ? `linear-gradient(90deg, ${stage.color}cc, ${stage.color})` : 'transparent', transition: 'width 0.6s ease', boxShadow: stage.count > 0 ? `0 0 8px ${stage.color}50` : 'none' }} />
              </div>
            </div>
          )
        })}
...
function WeeklyJobsChart({ jobsThisWeek }: { jobsThisWeek: any[] }) {
  ...
  const byDay = days.map(day => {
    ...
    return { day, label: DAY_LABELS[new Date(day + 'T00:00:00').getDay()], total: jobs.length, scheduled: jobs.filter((j: any) => j.status === 'scheduled').length, in_progress: jobs.filter((j: any) => j.status === 'in_progress').length, complete: jobs.filter((j: any) => j.status === 'complete').length }
  })
          ...
          {byDay.map(d => {
            ...
            return (
              <div key={d.day} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: d.total > 0 ? '#94a3b8' : '#1e3a4f', height: 16 }}>{d.total > 0 ? d.total : ''}</div>
                ...
                <div style={{ fontSize: 10, fontWeight: todayFlag ? 800 : 600, color: todayFlag ? '#0d7ea3' : '#475569', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{d.label}</div>
              </div>
            )
          })}
        </div>
    ...
    <div style={{ background: '#0f1923', border: '1px solid #1e3a4f', borderRadius: 12, overflow: 'hidden' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', background: 'linear-gradient(135deg,#162232 0%,#0d1a26 100%)', borderBottom: '1px solid #1e3a4f' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            ...
            <div style={{ color: '#475569', fontSize: 11, marginTop: 1 }}>{new Date().toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</div>
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)' }}>
        {days.map((day, i) => {
          ...
          return (
            <button key={day} onClick={() => setSel(isS ? null : day)} style={{ padding: '12px 4px 10px', borderRight: i < 6 ? '1px solid #1e3a4f' : 'none', borderBottom: isS ? '2px solid #0d7ea3' : '1px solid #0d1a26', background: isS ? 'rgba(13,126,163,0.12)' : tf ? 'rgba(255,255,255,0.02)' : 'transparent', cursor: 'pointer', textAlign: 'center' }}>
              <div style={{ fontSize: 9, color: '#334155', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 5 }}>{DAY_LABELS[new Date(day + 'T00:00:00').getDay()]}</div>
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
        <KPICard label="Active Leads" value={String(activeLeadCount)} sub={newLeadsToday.length > 0 ? `+${newLeadsToday.length} new today` : 'No new today'} icon="⬡" bg="linear-gradient(135deg, #0c2a4a 0%, #0f3460 100%)" border="#1d4ed8" accent="#38bdf8" onClick={() => navigate('/leads')} />
        <KPICard label="Today's Installs" value={String(todaysJobs.length)} sub={inProgressToday > 0 ? `${inProgressToday} in progress` : scheduledToday > 0 ? `${scheduledToday} scheduled` : 'None today'} icon="🔧" bg="linear-gradient(135deg, #2d1a00 0%, #3d2200 100%)" border="#92400e" accent="#f59e0b" onClick={() => navigate('/installations')} />
        <KPICard label="Jobs This Week" value={String(jobsThisWeek.length)} sub={`${completedThisWeek} completed`} icon="📅" bg="linear-gradient(135deg, #1e1040 0%, #251355 100%)" border="#6d28d9" accent="#a78bfa" onClick={() => navigate('/dispatch')} />
        <KPICard label="Failed Payments" value={String(failedPayments.length)} sub={failedPayments.length > 0 ? 'Needs immediate action' : 'All clear'} icon="💳" bg={failedPayments.length > 0 ? 'linear-gradient(135deg, #3b0a0a 0%, #450c0c 100%)' : 'linear-gradient(135deg, #0a2010 0%, #0c2a14 100%)'} border={failedPayments.length > 0 ? '#7f1d1d' : '#b91c1c'} accent={failedPayments.length > 0 ? '#ef4444' : '#4ade80'} urgent={failedPayments.length > 0} onClick={() => navigate('/customers')} />
      </div>
          ...
          <ActionCard title="Low Inventory" icon="📦" count={lowInventory.length} accent="#2dd4bf" urgent={lowInventory.length > 0} onClick={() => navigate('/inventory')}>
            {lowInventory.slice(0, 5).map((item: any) => (
              <Row key={item.id} onClick={() => navigate('/inventory')}>
                <RowLeft primary={(item.products as any)?.name || item.sku || 'Unknown item'} secondary={`${item.sku || 'No SKU'} · reorder at ${item.reorder_point}`} />
                <RowRight>
                  <div style={{ textAlign: 'right' }}>
...
function KPICard({ label, value, sub, icon, bg, border, accent, urgent, onClick }: {
  ...
  return (
    <button onClick={onClick} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 14, padding: '20px 20px 18px', textAlign: 'left', cursor: 'pointer', width: '100%', position: 'relative', overflow: 'hidden', boxShadow: urgent ? `0 0 20px ${accent}20` : 'none' }}
...
function ActionCard({ title, icon, count, accent, urgent, children, onClick }: {
  title: string; icon: string; count: number; accent: string; urgent?: boolean; children: React.ReactNode; onClick: () => void
}) {
  return (
    <div style={{ background: '#0f1923', border: `1px solid ${urgent && count > 0 ? `${accent}35` : '#1e3a4f'}`, borderLeft: `3px solid ${urgent && count > 0 ? accent : '#1e3a4f'}`, borderRadius: 12, overflow: 'hidden' }}>
      <div onClick={onClick} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 16px', background: 'linear-gradient(135deg,#162232 0%,#0d1a26 100%)', borderBottom: '1px solid #0d1a26', cursor: 'pointer' }}>
        ...
        <span style={{ minWidth: 26, height: 22, borderRadius: 20, padding: '0 8px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, background: count === 0 ? '#1e293b' : `${accent}18`, color: count === 0 ? '#334155' : accent, border: `1px solid ${count === 0 ? '#1e3a4f' : `${accent}25`}` }}>{count}</span>
      </div>
      <div>{children}</div>
    </div>
  )
