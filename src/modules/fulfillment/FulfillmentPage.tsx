...
interface TabDef {
  ...
  label: string
  icon: string
  filter: FulfillmentStatus[] | null // null = all
  color: string
}
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', justifyContent: 'flex-end' }}>
      ...
      <div style={{
        ...
        {isOverdue && (
          <div style={{
            ...
            <div style={{ fontSize: 13, fontWeight: 600, color: '#f87171' }}>
              ⚠️ Overdue — {daysPending} days since payment
            </div>
            <div style={{ fontSize: 11, color: '#fca5a5', marginTop: 4 }}>
              Customer paid but has not been contacted to schedule service.
            </div>
          </div>
        )}
        ...
        <div style={{
          ...
          <button
            onClick={() => navigate(`/customers/${fr.customer_id}`)}
            style={{
              marginTop: 10, fontSize: 11, color: '#0d7ea3', background: 'none', border: 'none',
          ...
          <div style={{
            ...
            <input
              ...
              style={{
                ...
                background: '#162232', color: '#e2e8f0', border: '1px solid #1e3a4f',
                marginBottom: 10,
              }}
            />
            <button
              ...
              style={{
                ...
                background: '#0d7ea3', color: '#fff', border: 'none', cursor: 'pointer',
                opacity: !scheduleDate || scheduleMut.isPending ? 0.5 : 1,
              }}
            >
              {scheduleMut.isPending ? 'Scheduling...' : '📅 Mark Scheduled'}
            </button>
            <div style={{ fontSize: 11, color: '#64748b', marginTop: 6 }}>
              Tip: Create a dispatch job first, then come back and link it here.
            </div>
          </div>
        )}
        ...
        {fr.status === 'scheduled' && (
          <button
            ...
            style={{
              ...
              marginBottom: 12, opacity: inProgressMut.isPending ? 0.5 : 1,
            }}
          >
        ...
        {fr.status === 'in_progress' && (
          <button
            ...
            style={{
              ...
              marginBottom: 12, opacity: completeMut.isPending ? 0.5 : 1,
            }}
          >
        ...
        {/* Cancel (for awaiting + scheduled only) */}
          ...
          <>
            {!showCancel ? (
              <button
                ...
                style={{
                  width: '100%', padding: '10px 0', borderRadius: 8, fontSize: 12, fontWeight: 600,
              ...
              <div style={{
                background: '#0f1923', borderRadius: 10, border: '1px solid rgba(248,113,113,0.25)',
                padding: 16,
              }}>
                <input
                  type="text"
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  ...
                  <button
                    ...
                    style={{
                      flex: 1, padding: '8px 0', borderRadius: 8, fontSize: 12, fontWeight: 600,
                      background: '#1e3a4f', color: '#94a3b8', border: 'none', cursor: 'pointer',
                    }}
                  >
                    Never mind
                  </button>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: '#64748b', margin: '4px 0 0' }}>
            Customers who paid — schedule their service
          </p>
        </div>
        <input
          type="text"
          ...
          return (
            <button
              ...
              style={{
                ...
                ...(isActive ? { border: '1px solid rgba(13,126,163,0.3)' } : { border: '1px solid #1e3a4f' }),
              }}
            >
              <span>{tab.icon}</span>
              <span>{tab.label}</span>
              {count > 0 && (
                <span style={{
                  ...
                  background: isActive ? 'rgba(13,126,163,0.25)' : 'rgba(148,163,184,0.1)',
                  color: isActive ? '#0d7ea3' : '#64748b',
                }}>
                  {count}
                </span>
              )}
      ...
      {isLoading ? (
        ...
        <div style={{
          ...
          <div style={{ fontSize: 14, fontWeight: 600 }}>
            {activeTab === 'awaiting' ? 'All caught up!' : 'No requests found'}
          </div>
          <div style={{ fontSize: 12, marginTop: 4 }}>
            {activeTab === 'awaiting'
              ? 'No customers waiting to be scheduled right now.'
              : 'No fulfillment requests match this filter.'}
          </div>
        </div>
      ) : (
          ...
          {filtered.map((fr) => {
            ...
            return (
              <div
                ...
                style={{
                  background: '#162232', borderRadius: 10, padding: '14px 18px',
                  border: isOverdue ? '1px solid rgba(248,113,113,0.35)' : '1px solid #1e3a4f',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: '#e2e8f0' }}>
                      {fr.customers?.full_name || 'Unknown'}
                    </span>
                    <StatusBadge status={fr.status} />
                    {isOverdue && (
                      <span style={{ fontSize: 10, color: '#f87171', fontWeight: 700 }}>
                        ⚠️ {daysPending}d overdue
                      </span>
                    )}
                  </div>
