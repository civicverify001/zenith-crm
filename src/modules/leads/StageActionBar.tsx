...
const STAGE_ACTIONS: Partial<Record<LeadStage, ActionDef[]>> = {
  ...
  site_visit_scheduled: [
    { label: 'Open Quote', action: 'move', variant: 'primary', targetStage: 'proposal_in_progress' },
    { label: 'Lost', action: 'lost', variant: 'danger', modal: 'lost' },
    { label: 'Follow-Up', action: 'followup', variant: 'secondary', modal: 'followup' },
    { label: 'DND', action: 'dnd', variant: 'warning', modal: 'confirm_dnd' },
  ],
  proposal_in_progress: [
    { label: 'Send Quote', action: 'move', variant: 'primary', targetStage: 'quote_sent' },
    { label: 'Lost', action: 'lost', variant: 'danger', modal: 'lost' },
    { label: 'Follow-Up', action: 'followup', variant: 'secondary', modal: 'followup' },
  ],
  quote_sent: [
    { label: 'Agreement Signed', action: 'agreement', variant: 'success', modal: 'agreement' },
    { label: 'Lost', action: 'lost', variant: 'danger', modal: 'lost' },
    { label: 'Follow-Up', action: 'followup', variant: 'secondary', modal: 'followup' },
  ],
  agreement_signed: [
    {
      label: 'Create Install Job', action: 'install_job', variant: 'primary', modal: 'install_job',
      disabled: (lead) => !!lead.job_created, disabledLabel: 'Job Created ✓'
    },
    { label: 'Lost', action: 'lost', variant: 'danger', modal: 'lost' },
  ],
  lost: [
...
export function StageActionBar({ lead, onLeadUpdated }: Props) {
...
if (!user) return null
  ...
  async function handleAction(actionDef: ActionDef) {
    setError('')
    // If action needs a modal, open it
    if (actionDef.modal) {
  ...
  async function handleReopen() {
    ...
    try {
      let updated: Lead
      if (lead.stage === 'lost') {
        updated = await reopenFromLost(lead.id, lead, actor)
      } else if (lead.stage === 'dnd') {
        updated = await reopenFromDND(lead.id, actor)
      } else {
        updated = await reopenFromFollowUp(lead.id, lead, actor)
      }
      onLeadUpdated(updated)
      setActiveModal(null)
    } catch (e: any) {
    ...
    <>
      ...
      {activeModal === 'followup' && (
        <FollowUpModal
          ...
          onCancel={() => setActiveModal(null)}
          isPending={pendingAction === 'followup'}
        />
      )}
      {activeModal === 'agreement' && (
        <AgreementModal
          ...
          onCancel={() => setActiveModal(null)}
          isPending={pendingAction === 'agreement'}
        />
      )}
      ...
      {activeModal === 'confirm_reopen' && (
        <ConfirmDialog
          ...
          message={
            lead.stage === 'lost'
              ? 'This will reopen the lead as New Lead and clear the lost reason.'
              : lead.stage === 'dnd'
              ? 'This will reopen the lead as New Lead and remove the DND status.'
