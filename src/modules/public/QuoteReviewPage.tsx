  ...
  async function handleAction(action: 'accept' | 'decline') {
    ...
    try {
      const res = await fetch(`/api/quotes/review?token=${token}`, {
        ...
        body: JSON.stringify({ token, action, decline_reason: declineReason || undefined, signature: signature || undefined }),
      })
      ...
      {result && (
        <div style={{ maxWidth: 800, margin: '24px auto 0', padding: '0 20px' }}>
          <div style={{
            ...
            border: `1px solid ${result.status === 'accepted' ? '#86efac' : '#fecaca'}`,
          }}>
            ...
            <div style={{ fontSize: 15, color: '#475569', maxWidth: 480, margin: '0 auto' }}>
              {result.status === 'accepted'
                ? 'Thank you for choosing Zenith Pure Solutions! Our team will contact you shortly to schedule your installation.'
                : 'Thank you for letting us know. Please contact us if you change your mind or have any questions.'}
            </div>
            {result.status === 'accepted' && quote.commercial_type === 'rental' && (
              <div style={{ marginTop: 16, padding: '12px 16px', background: '#ecfdf5', borderRadius: 8, border: '1px solid #a7f3d0', fontSize: 13, color: '#065f46' }}>
                A rental agreement has been prepared. Our team will reach out for signature.
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ background: '#fff', borderRadius: 4, boxShadow: '0 1px 4px rgba(0,0,0,0.08)', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '28px 40px 20px' }}>
            ...
            <div style={{ display: 'flex', gap: 48, marginTop: 16, flexWrap: 'wrap' }}>
              ...
              <div>
                ...
                <div style={{
                  display: 'inline-block', marginTop: 4, padding: '2px 10px', borderRadius: 4, fontSize: 12, fontWeight: 700,
          ...
          <div style={{ padding: '0 40px 24px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {quote.line_items.map((li: any, i: number) => (
                  <tr key={i} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '12px 8px', fontSize: 13, color: '#64748b', fontFamily: 'monospace' }}>{li.sku || '—'}</td>
              ...
              <tfoot>
                <tr><td colSpan={3} /><td style={{ padding: '8px', fontSize: 13, color: '#64748b', textAlign: 'right' }}>Subtotal</td><td style={{ padding: '8px', fontSize: 13, fontWeight: 600, color: '#1e293b', textAlign: 'right' }}>{fmt(quote.subtotal)}</td></tr>
                <tr><td colSpan={3} /><td style={{ padding: '8px', fontSize: 13, color: '#64748b', textAlign: 'right' }}>Taxes</td><td style={{ padding: '8px', fontSize: 13, fontWeight: 600, color: '#1e293b', textAlign: 'right' }}>{fmt(quote.tax_amount)}</td></tr>
                <tr style={{ borderTop: '2px solid #0c4a6e' }}><td colSpan={3} /><td style={{ padding: '12px 8px', fontSize: 15, fontWeight: 800, color: '#0f172a', textAlign: 'right' }}>Total</td><td style={{ padding: '12px 8px', fontSize: 18, fontWeight: 800, color: '#0f172a', textAlign: 'right' }}>{fmt(quote.total)}</td></tr>
              </tfoot>
            </table>
          </div>
          ...
          <div style={{ padding: '24px 40px' }}>
            ...
            <div style={{ fontSize: 12, color: '#475569', lineHeight: 1.8 }}>
              ...
              <a href="https://zenithpuresolutions.com/terms" target="_blank" rel="noopener noreferrer" style={{ color: '#0c4a6e', fontWeight: 600, textDecoration: 'underline', display: 'inline-block', marginTop: 4 }}>
                ...
                {!showDecline ? (
                  <div>
                    ...
                    <div style={{ marginBottom: 20 }}>
                      <label style={{ display: 'block', fontSize: 11, fontWeight: 700, color: '#64748b', letterSpacing: '0.06em', marginBottom: 6 }}>DIGITAL SIGNATURE (type your full name)</label>
                      <input type="text" value={signature} onChange={e => setSignature(e.target.value)} placeholder="e.g. John Smith"
                        style={{ width: '100%', maxWidth: 360, padding: '12px 16px', fontSize: 18, fontFamily: '"Brush Script MT", "Segoe Script", cursive', border: '1px solid #cbd5e1', borderBottom: '2px solid #0f172a', borderRadius: 4, outline: 'none', boxSizing: 'border-box', background: '#fafafa' }} />
                    </div>
                    <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
                      <button onClick={() => handleAction('accept')} disabled={busy || !agreedTerms || !signature.trim()}
                        style={{ padding: '14px 48px', borderRadius: 6, border: 'none', cursor: agreedTerms && signature.trim() ? 'pointer' : 'not-allowed', fontSize: 15, fontWeight: 700, color: '#fff', background: agreedTerms && signature.trim() ? '#16a34a' : '#94a3b8', opacity: busy ? 0.6 : 1, boxShadow: agreedTerms && signature.trim() ? '0 2px 8px rgba(22,163,74,0.3)' : 'none', transition: 'all 0.2s' }}>
